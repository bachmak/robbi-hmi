...
The backend is organized into four layers. The \textbf{API layer} exposes an HTTP interface that
Grafana uses to submit commands. The \textbf{domain layer} contains the business logic, including
the kinematic calculations that convert linear and angular velocity into individual wheel speed
setpoints. The \textbf{infrastructure layer} holds the two outward-facing adapters: the OPC UA
client, which connects to the PLC, subscribes to value-change notifications on all leaf nodes for
continuous monitoring, and polls periodically to capture snapshots; and the InfluxDB client, which
persists incoming process data and command records. The \textbf{application layer} ties these
together, routing incoming commands from the API through the domain logic and out to both
